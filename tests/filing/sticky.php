...
if ( 6 === $sk_reach ) {
    ...
    // Nathan's words (d661cf6, option A, no 'AI'): the agree row 'In X · both matches agree' with no score line, the doubt row 'Left where it was · matched X, but in doubt' then only looked at.
    sk_check( 'J6b the why card says the shipped words and nothing more: the agree row reads In A · both matches agree (sure, no score line), the doubt row Left where it was · matched A, but in doubt and only looked at, the model-alone row as likely with no matched-word line', is_array( $sk_why_a ) && 'In zzStickyA · both matches agree' === (string) $sk_why_a['lines'][0] && ! preg_grep( '/scored /', $sk_why_a['lines'] ) && 'sure' === $sk_why_a['confidence'] && is_array( $sk_why_d ) && 'Left where it was · matched zzStickyA, but in doubt' === (string) $sk_why_d['lines'][0] && (bool) preg_grep( '/^Looked at /u', $sk_why_d['lines'] ) && ! preg_grep( '/^(In |Put here|Matched |Filed )/', $sk_why_d['lines'] ) && is_array( $sk_why_o ) && 'likely' === $sk_why_o['confidence'] && ! preg_grep( '/^Matched /', $sk_why_o['lines'] ), json_encode( array( 'a' => is_array( $sk_why_a ) ? array( $sk_why_a['confidence'], $sk_why_a['lines'] ) : null, 'd' => is_array( $sk_why_d ) ? $sk_why_d['lines'] : null, 'o' => is_array( $sk_why_o ) ? array( $sk_why_o['confidence'], $sk_why_o['lines'] ) : null ) ) );
