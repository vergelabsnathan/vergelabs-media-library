...
test.describe( 'the Folders screen', () => {
	...
	test( 'a tree from the conversation counts before it shows numbers', async ( { page } ) => {
		...
		await expect( tree.locator( '.g-pill.is-quiet', { hasText: 'counts not worked out yet' } ), 'the row says it is counting; the quiet pill is for a count that came back unknown' ).toHaveCount( 0 );
