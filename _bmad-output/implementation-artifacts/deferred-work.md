...
- source_spec: `spec-e3-1-the-service-away-pictures-wait.md` (build, 2026-09-22)
  summary: the `ai` UI suite (tests/tree/ai.mjs) describes three real pictures in demo mode and leaves mock index rows and mock alt text on them. The newest row in the library is then a mock, so `ai-background`'s G1 reads the stamp as another model on a mock prompt hash and goes red, and the box's search and filing hold three demo captions until someone notices. It should snapshot the three rows (literal SQL) and put them back, or describe them for real in its teardown.
  evidence: G1 red twice on 2026-09-22 with `stale: 3`; fixed by hand both times (3 credits each, 6 in total).
- source_spec: `spec-e3-1-the-service-away-pictures-wait.md` (build, 2026-09-22)
