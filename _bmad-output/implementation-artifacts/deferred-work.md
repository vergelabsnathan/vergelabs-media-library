...
- source_spec: `spec-4-2-the-compatibility-matrix-on-4-0-0.md`
  summary: beside FileBird 6.5.8 on the box's MariaDB network, a single unselected row dragged onto a folder lights the folder and files nothing -- our draggable's helper() (which sets `dragging`) does not run for that press, so the drop handler (js/vergeml-tree.js:2310) falls back to the checked rows and finds none; a drag of checked rows files them. The 09-11 bytes passed the same cell, so a commit between c85cde6 and 4.0.1 undid the arming in armOne() (js/vergeml-tree.js:2073-2082) -- the two 09-17 list commits (aedb0ef S10.6, 3a13b74 takeNav) are the candidates. A story of its own; the cell is the proof (`node tools/matrix.mjs --cell with=filebird,shape=multisite-subdirectory`).
  evidence: matrix 2026-09-20 on 4.0.1: 6/9, "drag one into the folder: the folder lit up; file 251050 is in []", then "2 of 2 moved; the folder counts 2"; the same 6/9 by hand with mock off; the subdirectory cell without FileBird 9/9 a minute earlier.
- source_spec: `spec-4-2-the-compatibility-matrix-on-4-0-0.md`
