...
- **S13 (done 2026-09-17, `docs/handoffs/2026-09-17-s13-the-fill-learns.md`)** — S10.7 the fill learns from its own placements (a folder of three or more read over its members' words two or more say, the word owned by the folder holding most of them, a locked folder owning none; round 2 over the leftovers); S10.9 the picture's own words (file and title; the alt only when a person wrote it — the describer's doubled the ties); S10.5 rule 3 (the head noun capped only where the class half is the phrase's own modifier and names a folder by its first class or leaf). Tech read dry: sure 340 → 637, floor 370 → 147, margin 36 → 104; shop: sure 467 → 491, siblings 25 → 16, margin 60 → 71. The number is not the verdict: the batch-133 sheet re-read says the folders hold the fill's own misses (sure: right kept 13, wrong kept 13). Nathan's marks on the S13 sheet: sure 25/30, likely 6/30 (C.4: 26/30, 17/30) — the likely band is the learned misses; the tech band re-taken on that, the shop's waits for its sheet. S10.5b not reached (Nathan's yes on the catalogue).
- **S14 (2026-09-17, `docs/handoffs/2026-09-17-s14-words-corroborate.md`)** — the member trust rule built, judged dry on both sheets and taken out (3 of 17 on tech; −21 sure and seven right placements lost on the shop: a placement's confidence does not say whether the word it teaches is true); the cause of the likely band found and fixed: S10.9's words placed alone where the describer hit nothing — now corroboration only (tech 686/637 → 655/599, the S13 sheet's likely 10 of 17 wrong dropped, 1 right lost; the shop 510/488, its C.5 marks re-read by path for the first time — `bfq_path` read `&amp;`); both bands re-taken with the reason; guide F7's race with F6's real cron job answered in-suite; the two UI reds were lagging assertions ("would be placed", the confirm as the done step's primary) and a restore cascade past the cap. Then, on Nathan's yes, S10.5b: HEMA's tree (292 folders, Dutch) over the shop's 626 — 34 folders profiled for 0 credits, the fill 440 placed / 196 sure / 244 likely / 186 in no folder, dry 449 / 255 / 194 / 177 against the C.5 tree's 510 / 488 / 31 / 107 (the honest range for sure: 41 % to 78 %, tech 60 %); every sure a top-level folder, *nieuwe collectie* took 212; the C.5 tree frozen with its ids (`tools/box-tree-snapshot.php`). S10.8: not reached.
- **S15 (done 2026-09-17, `docs/handoffs/2026-09-17-s15-library-words-and-views.md`)** — the electronics question probed first (93 pictures say it, none in Batteries, which holds it from the planner); a class half the library says is a library word (k counts the folders whose members say it, the class equal to or inside what they say) and the class half inside a folder's specific phrase is no hit — tech 655/599 → 645/594, margin 64 → 27, the S13 sheet's likely wrong dropped 10 → 13, sure wrong kept 5 → 4, 13 e-waste sures out of Batteries; HEMA's findings: a class-count rule judged dry and not built (round 1 fits 422 → 140), a view of the tree owns nothing (sale, nieuwe collectie: 184 → 0, the shop-b band 258 → 186 sure, the honest 30 %), the foreign leaf sent to the planner on Nathan's yes (ms2 set to nl_NL, 27 credits, 64 leaves named; round 1 sure 159 -> 195 then k by lines built the same evening: HEMA sure 180 -> 227, tech sheets identical; the C.5 tree back, Watches read as a view until audience-named children stopped counting, its band 495/479); the apply's 150 s were one `/embed` a folder for nothing (removed, `guide.php` G2) and the row says "Making 292 folders · 4 s"; S10.8's premise probed (no map exists, no product-bearing picture on either site) — the S16 card's first task.
- **S16 (done 2026-09-18, `docs/handoffs/2026-09-18-s16-the-truth-score-first.md`)** — the truth score as the gate (`tools/box-truth-score.php` on the shop, every rule judged by its SCORE line): 330 of 581 right (57 %), right of placed 69 % → **348 (60 %), 71 %**. Its top three losers probed first: two *Backpacks* tie (namesakes), Bookcases tie with Shelving (the planner's *public bookcase*), men's sweaters and jackets **gated by audience** (44 of 581 have their own folder gated — the largest loss left, Nathan's call). Built, one story each with a red mutation: the `lens` canon defect (+3); a member word only where alike to the folder's own at the members' floor 0.5 (Nathan's S15 verdict: wrong sures 7 → 3, none lost); a one-word class as the class half's head noun is no hit (wrong likelies 9 → 4); a folder's name claims the planner classes it names (+15: lenses, bookcases; Interviews' *people*); the pick says its source and the trail keeps it (`source`, `hit`, schema 4; the shop by source: members 80 % right, name 66 %, the planner's own words 2 of 15); S10.8 tasks 1–3 (the map, the fact, the live woo case on its own pictures) and task 4 mocked; the ask leaves a view home. Probed and not built: the class floor (7 placements stand on it now). Not reached: the audience words on HEMA (ms2's tree), the tech truth page (shape proposed).
- **S17** — S10.10 the proposal from the pictures: groups to name and nest
