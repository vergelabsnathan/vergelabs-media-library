...
- **S15** — the class-half hits (the 7 wrong likelies left: "gaming hardware" → hardware, "technical diagram" → diagram); then S10.8 file by the product (WooCommerce) and the flipped order
  of the steps for structured libraries (the rail mocked); S10.11 the
  opt-in model pass on what is left.
- **S16** — S10.10 the proposal from the pictures: groups to name and nest
