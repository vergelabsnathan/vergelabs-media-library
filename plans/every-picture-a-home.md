...
- **S16** — the audience words in the site's language and the two profile findings (Interviews claims people first, TV & Video camera lens); S10.8 file by the product (the map, the fact, the pick, the live test on the tech site's own product; the rail's "placed by product N"), then the flipped order for structured libraries (mocked); the class floor for vector-only class-half matches (the 3 left of the 7).
