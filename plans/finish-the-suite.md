...
## The release train, 4.0.3 — done 2026-09-22 (Opus 5, S32)

The 4.0.3 story (E4-1, E4-3, E3-2; `spec-4-0-3-retro-fixes.md`, 24 review rows) from `82bd06d`, on Nathan's "go and finish it": D1–D3 applied as proposed (the temporary-error wording, the next-pass wording, the incident template), D4 left. Evidence, in the train's order: plugin `6380f33` (readme: Stable tag, the 4.0.3 changelog — the handoff's strapline and two Fixed lines; header and `VERGEML_VERSION`), `82bd06d` (Playground zip); `archive-hygiene` **8/8**; Plugin Check on `git archive` of `82bd06d` in Playground (CLI 3.1.54), PHP 8.3, five categories: **0 errors, the 2 known warnings**. Tag `v4.0.3` on `82bd06d`; `dist/vergelabs-media-library-4.0.3.zip` = `git archive v4.0.3`, **143 entries, 1,103,004 bytes, sha256 `95ec481210d9`**, header `4.0.3`, no hidden entry. Plugin `main` pushed `a3987c2..82bd06d` (fast-forward; the watch had not committed), `box.yml` put it on the box (`deploy.mjs --check`: 136 files verified). GitHub release `v4.0.3` **latest**, asset read back `200 · 95ec481210d9 · 1103004 · Version: 4.0.3 · 143 entries`. Service: the shelf commit `9664331` in a worktree at `origin/main` with the two docs commits cherry-picked, pushed as `main` alone (the checkout keeps the pricing commits and migration 020); `release-files` **11 passed**, `pnpm test` **545 passed | 14 skipped**; HTTPS read-back the same line. Catalogue parsed by `releases()` (both rows; Pro's unchanged), env swapped 07:31:43 → :50 UTC, redeploy `p789l2146` aliased in 50 s, `promote.mjs` both hostnames. Health **`free 4.0.3, pro 1.0.3`**; `4.0.2 → update:true` with the versioned package, `4.0.3 → update:false`; release-check **`200 ok:true`** both slugs (07:34 UTC). Nothing spent; nothing on ms2 or the real shop.
...
## Wave 5 — the decision sitting, held 2026-09-21 (Opus 5, S30); all seven answered

`docs/decisions-2026-09.md`: seven rows, the evidence gathered by a read across the repos, one recommendation each; Nathan answered six in one message. Written where they live: **2** one business day stays, `support@vergelabsmedia.com` the one address — readme.txt Support (the ticket mail's sentence verbatim) and the site's support heading (service `0317178`, live, 0 mentions of the old address on the page); 3.3 → `done`. **3** client-grade with the pause (500 / first 100) — `epics.md` 4.3; a mock is the next step. **4** PRIVATE0 off — the deferred row marked decided; the two clicks are Nathan's. **5** Santa Cruz is the registered office — `vat.md` item 5; Stripe's business profile is Nathan's field. **6** UK and US tax per the rules for a Spanish entity, the accountant states them, no checkout hold — `vat.md` item 6 (the recommended hold not taken). **7** retros recorded, headless, in wave 6 after row 1. **1** answered after the sitting ("yes and let users know about it"): the outage answer written for users — readme.txt FAQ "What happens when the AI service is down?" and the credits docs page (service `299adc1`, live), from the terms line and the incident runbook's approved sentence; 3.1 → `done`. Epic 3 is fully answered; epic 4 waits on the 4.3 mock; epic 2 on the accountant. Also this session: the Playground CLI pinned in `tools/lib/playground.mjs` (3.1.54; `csv-local` 69/69 on the pin).

## Wave 6 — done 2026-09-21 (Opus 5, S31): 2.1's review, the retros for epics 3 and 4

`bmad-code-review` on story 2.1 (plugin `e106e20..HEAD` walk tools, service `f581f56` + the two scripts; 5 files, +580/−1): four lenses, 19 rows in the spec's triage log, **17 patched** (plugin `59f67ca`, service `33063ea` on the checkout branch), 1 decision (row 7, the `forgot` sentence — Nathan's), 1 deferred (the PDF read), 21 rejected. Proof: `buyer-walk-site.mjs snapshot` on `upg` under a fresh run wrote 5 rows, sha256 `19cba051d577` (wave 4's by-hand digest), `restore` against it **0 differences**; `e2e/auth.spec.ts` **2 passed** against production. 2.1 → `done`, 4.2 → `done`. Then `bmad-retrospective -H` for both epics from one `bmad-review` pass over the combined diff (2,394 lines; adversarial 14, edge-case 15, verification-gap 6), re-checked at the source: **epic 3 accepted-with-open-items** (`epic-3-retro-2026-09-21.md`, 9 action items) — the largest finding is that the outage FAQ shipped in S30 overstates `core/ai.php`: a transport failure is stubbed permanently on the first miss (`:1568-1570`) and a 5xx outage stubs after the third strike, so pictures are *not* described "when it is back" without a re-describe; a story (E3-1) and a qualified sentence (E3-2, Nathan's words). **Epic 4 rejected** by the machine rule (4.3 decided, not built; `epic-4-retro-2026-09-21.md`, 6 action items) — and one shipped defect found by a geometry probe on `upg`: with Screen Options open in list mode, 3 of 4 column checkboxes sit under the fixed tree panel (`#screen-meta` z 2, the panel z 3; shot `2026-09-21-screen-options-under-tree-upg.png`); E4-1, 4.0.3 material. `sprint-status.yaml`: both retro keys `done`, 15 action items appended, epic 3 `done`. Nothing spent; nothing on ms2 or the real shop; nothing pushed.
