...
The text model's word beside the rules (S18, `plans/agree-or-ask.md`). `POST` from the plugin server-side with `license_key`, `site`, `folders` (the tree as paths `"Parent > Child"`, at most 500, *To sort* never among them) and `pictures` (`[{ id, says, caption }]`, at most 40 a call: `says` the describer's object phrases, `caption` its caption, no image). Answers `{ answers: { "<id>": "<path>" | null }, model }`: for every asked id the one folder path of the list that fits, spelled as the list spells it, or `null` for "nothing of this tree fits"; a path not in the list is `null`. Sonnet 5 through OpenRouter at temperature 0, the prompt of `tools/truth-model.mjs`. Authenticated as `/v1/name-group` (`401 bad_key`, `400 invalid_site`, `403 not_found | not_entitled | site_not_activated`, `429` on the limits with `Retry-After`); `400 invalid_body | too_many_folders | too_many_pictures | empty_tree | empty_batch`; `502 filing_unavailable` when the model call fails (the plugin then files rules-only and leaves the service alone for a minute). Metered on `file` at cost 1 a call and never debited until the call is priced; no description is logged. The plugin asks once per picture per tree and keeps the answer a week (`vergeml_filing_ask_model`, `core/filing.php`); the dry count never asks.
