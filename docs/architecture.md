...
The Folders screen (`js/vergeml-folders.js`, `core/guide.php`): one conversation, one tree, one Move. The browser streams each turn straight from the service (`/v1/guide/stream`, server-sent events) with a one-hour token `core/guide.php` mints with the licence key (`/guide/token`); the key never reaches the browser. Each finished turn is persisted (`/guide/turn`) into the option `vergeml_guide_session` with the draft keyed by term id, so a reload shows the same conversation and tree. The tree is the shared component `js/vergeml-tree-view.js`. The Rules tab (`/guide/rules`, `/guide/rule`) builds the same draft four deterministic ways without a model; a rule's Move carries an explicit assignment. Move (`/guide/apply`) goes through `vergeml_talk_apply()` and the resumable re-filing in `core/folder-talk.php`, with Stop (`/guide/stop`) and a day of undo (`/guide/undo`).
