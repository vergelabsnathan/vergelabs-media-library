# S30 — Wave 5: the decision sitting, six of seven answered
...
commit); service `origin/main` `fc940b9` → `1f92502` (serving). **Spent:
...
okay". Row 1 not answered.

## Answers and where they landed

| Row | Answer | Landed |
|---|---|---|
| 1 · 3.1 outage | — | open; the proposal stands (adopt what the code does, write it in the readme FAQ and the credits page) |
...
| 7 · retros | recorded, headless | wave 6, after row 1; 2.1's code review first |
...
- Service work again went through a worktree at `origin/main` pushed as
  ...
  now `efc9143`, `6094875`, still unpushed.
...
~/.claude/harness/model-profiles.md. This session: write row 1's answer
where it lives (my answer: <…>), then bmad-code-review on story 2.1, then
