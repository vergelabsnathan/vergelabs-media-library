...
walk's three rules-only assumptions rewritten the same evening: E3 (the
preview is the estimate: rules_only, same looked and kept), F1 (a tie is a
question either way: siblings or either/or), F5 (To sort = what it held +
what leave left; an either/or card keys its pictures, which F5 had read on
