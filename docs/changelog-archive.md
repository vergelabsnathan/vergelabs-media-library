...
Everything before 3.7.0, moved out of `readme.txt` on 29-08-2026.

Not a tidy-up. wordpress.org truncates a Changelog section over 5,000
characters and Plugin Check reports it as a warning, and the full history
had reached 30,708 — six times the limit. A truncated changelog is worse
than a short one, because nobody can tell where it stopped.

So `readme.txt` keeps the three most recent releases and points here for the
rest. Nothing is lost: the Enhanced Media Library releases this fork
continues are below, unedited, including the 2.9.x entries written by
wpUXsolutions.
