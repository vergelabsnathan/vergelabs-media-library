...
| `/v1/support/ticket` | site URL, a persistent per-site token, **the typed question**, **an email address**, the matched known issues, **the plaintext licence key**, and **the whole system report** — home URL, WP / PHP / MySQL versions, server limits, debug flags, theme, attachment count, every attachment taxonomy with its term count, fifteen plugin settings, and **the name and version of every active plugin** | the Send button on Get help: `manage_options`, nonce, a question, and a consent tick | **no** |
